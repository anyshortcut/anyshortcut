...
window.getBoundDomainByHostname = function (hostname) {
  for (let domain of Object.keys(secondaryShortcuts)) {
    if (
      secondaryShortcuts.hasOwnProperty(domain) &&
...
window.openShortcut = function (shortcut) {
  ...
  client
    .increaseShortcutOpenTimes(shortcut.id)
    .then((shortcut) => {
      if (shortcut.primary) {
...
window.removePrimaryShortcut = function (shortcut, including) {
  return new Promise((resolve, reject) => {
    client
      .unbindShortcut(shortcut.id, including)
      .then((response) => {
...
window.getPrimaryShortcutByDomain = function (domain) {
  for (let key of Object.keys(primaryShortcuts)) {
    if (primaryShortcuts.hasOwnProperty(key) && primaryShortcuts[key].domain === domain) {
      return primaryShortcuts[key];
    }
  }
  return null;
