<template>
  <div class="compound-keyboard">
    <div :style="{ top: -scrollTop + 'px' }" class="left-column">
      <span class="column-header" v-for="rowKey in alphanumeric" :key="rowKey">
        {{ rowKey }}
      </span>
    </div>
    ...
    <table>
      <thead :style="{ left: -scrollLeft + 'px' }">
        <tr>
          <th v-for="rowKey in alphanumeric" :key="rowKey" class="row-header">
            {{ rowKey }}
          </th>
        </tr>
      </thead>
      <tbody ref="tbody" @scroll="onScroll" id="compound-tbody">
        <tr v-for="rowKey in alphanumeric" :key="rowKey">
          <td
            :id="rowKey + columnKey"
