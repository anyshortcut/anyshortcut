...
chrome.storage.onChanged.addListener((changes, area) => {
  if (area === 'local' && changes[PREFS_KEY]) {
    cache = { ...DEFAULTS, ...((changes[PREFS_KEY].newValue as Partial<Prefs>) || {}) };
  }
});

function save<K extends keyof Prefs>(key: K, value: Prefs[K]): Promise<void> {
  cache[key] = value;
...
function readLegacyPrefs(): Partial<Prefs> | null {
  ...
  if (combinationKey !== null) legacy.combinationKey = combinationKey as CombinationKey;

  const showCircle = localStorage.getItem(LEGACY_KEYS.showCircle);
  if (showCircle !== null) legacy.showCircle = showCircle as ShowCircleConfig;

  for (const key of ['openByBlank', 'compoundEnable'] as const) {
...
export default {
  async init(): Promise<void> {
    ...
    const stored = result[PREFS_KEY] as Partial<Prefs> | undefined;
    if (stored) {
  ...
  getDefaultCombinationKey(): CombinationKey {
    return cache.combinationKey;
  },
  setDefaultCombinationKey(value: CombinationKey): Promise<void> {
    return save('combinationKey', value);
  },
  isShortcutOpenByBlank(): boolean {
    return cache.openByBlank;
  },
  setShortcutOpenByBlank(value: boolean): Promise<void> {
    return save('openByBlank', value);
  },
  isCompoundShortcutEnable(): boolean {
    return cache.compoundEnable;
  },
  setCompoundShortcutEnable(value: boolean): Promise<void> {
    return save('compoundEnable', value);
  },
  getShowCircleConfig(): ShowCircleConfig {
