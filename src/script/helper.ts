...
export default {
  delayTime: 200,
  ensureWindowEvent(e: KeyboardEvent): KeyboardEvent {
    return e ? e : (window.event as unknown as KeyboardEvent);
  },
  isValidKeyEvent(e: KeyboardEvent): boolean {
  ...
  isValidKeyCode(keyCode: number): boolean {
    return Boolean(dictionaries[keyCode]);
  },
  withoutAnyModifier(e: KeyboardEvent): boolean {
    return Boolean(e && !e.altKey && !e.shiftKey && !e.ctrlKey && !e.metaKey);
  },
  /*
   * Check the event key modifier is full valid or not.
   */
  withAltModifier(e: KeyboardEvent): boolean {
    // Ignore SHIFT key to compact key conflict
    return Boolean(e && e.altKey && !e.ctrlKey && !e.metaKey);
  },
  withShiftModifier(e: KeyboardEvent): boolean {
    // Ignore active element editable case.
    if (this.isActiveElementEditable()) {
      return false;
    }
    return Boolean(e && e.shiftKey && !e.altKey && !e.ctrlKey && !e.metaKey);
  },
  getEventModifier(e: KeyboardEvent): 'alt' | 'shift' | null {
